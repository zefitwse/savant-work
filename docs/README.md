...
- **动态推理频率策略 (Adaptive Inference)**：
  - 平时以 5fps 进行低频巡检，25fps 输入对应 `nvinfer interval=4`
  - 检测到疑似目标时，自动提升至 25fps 全帧率，运行时切换到 `nvinfer interval=0`
  - 获取更精准的轨迹和抓拍

- **静止过滤 (Motion Masking)**：
  - 在 Savant 前端引入简单的运动检测
  - 如果画面完全静止（如深夜的库房），进入 paused 并下发 `nvinfer interval=250`
  - 运动阈值可配置，低于阈值时进入省电模式
...
scripts/video_file_replay_loop.sh         无限循环视频重放脚本，默认 LOOP_COUNT=0；测试可显式设置 LOOP_COUNT=10
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| idle_fps | 5.0 | 空闲巡检帧率，25fps 输入对应 `nvinfer interval=4` |
| active_fps | 25.0 | 告警后全帧率推理，25fps 输入对应 `nvinfer interval=0` |
| motion_mask_enabled | true | 是否启用静止过滤 |
| motion_threshold | 50000.0 | 运动阈值，低于此值判定为静止 |
| alert_threshold | 100000.0 | 告警阈值，高于此值触发高频推理 |
| static_cooldown_frames | 50 | 静止持续多少帧后进入 paused |
| target_cooldown_frames | 125 | 检测目标后保持 25fps 的帧数，25fps 下约 5 秒 |
| paused_interval | 250 | paused 模式实际下发给 `nvinfer` 的近似暂停 interval |
| history_size | 10 | 运动历史窗口大小 |
| resize_width | 64 | 运动检测时图像缩放宽度 |
| resize_height | 64 | 运动检测时图像缩放高度 |

策略可通过 Control API 动态下发：
...
容器验证时可在 `runtime/savant-output/metadata.json` 或 `runtime/raw-output/metadata.json` 中查看 `adaptive_inference.mode`、`adaptive_inference.requested_fps`、`adaptive_inference.nvinfer_interval` 和 `adaptive_inference.effective_nvinfer_interval`。为避免 DeepStream 运行时重建 `nvinfer` 上下文导致 CUDA host buffer 分配失败，代码采用运行中直接更新 `interval` 属性的方式切换 5fps/25fps；paused 模式已验证可实际下发 `effective_nvinfer_interval=250`，metadata 中对应 `requested_fps=0.00` 和 `should_process=False`。
