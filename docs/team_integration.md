...
注意：测试视频由 `scripts/video_file_replay_loop.sh` 无限循环推送，默认 `LOOP_COUNT=0` 表示无限循环。需要临时测试固定轮数时，可以显式设置 `LOOP_COUNT=10`；一键脚本会后台启动 source adapter，采样完成后主动停止 `source-video` 和 `source-video-raw`，避免测试流程卡住。
...
```text
AdaptiveInferenceController 已实现运行时 nvinfer interval 动态切换：
  adaptive_inference.mode = paused / idle / alert
  adaptive_inference.should_process = True / False
  adaptive_inference.motion_score = 整数分数
  adaptive_inference.requested_fps = 0.00 / 5.00 / 25.00
  adaptive_inference.nvinfer_interval = 250 / 4 / 0
  adaptive_inference.effective_nvinfer_interval = 实际下发给 nvinfer 的 interval
...
| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `idle_fps` | `5.0` | idle 模式下按 5fps 低频巡检，25fps 输入对应 `nvinfer interval=4` |
| `active_fps` | `25.0` | 检测到疑似目标后进入 alert，全帧率推理，`nvinfer interval=0` |
| `motion_mask_enabled` | `true` | 是否启用静止过滤 |
| `motion_threshold` | `50000.0` | 低于该值并持续冷却后进入 paused |
| `alert_threshold` | `100000.0` | 运动分数高于该值进入 alert |
| `static_cooldown_frames` | `50` | 静止持续帧数 |
| `target_cooldown_frames` | `125` | 目标出现后保持 alert 的冷却帧数，25fps 下约 5 秒 |
| `paused_interval` | `250` | paused 模式的逻辑暂停标记值 |
| `history_size` | `10` | 运动分数历史窗口 |
| `resize_width` | `64` | 运动检测缩放宽度 |
| `resize_height` | `64` | 运动检测缩放高度 |

运行时策略通过 Control API 下发：
...
容器级验证结果：`docker compose -f docker-compose.savant.yml up -d --build --force-recreate ...` 后，masked/raw 两个 Savant module 均保持 healthy；日志中可观察到 `Applied adaptive nvinfer interval=4` 以及检测目标后 `Applied adaptive nvinfer interval=0`；`runtime/savant-output/metadata.json` 和 `runtime/raw-output/metadata.json` 中可见 `adaptive_inference.requested_fps=25.00`、`effective_nvinfer_interval=0`。alert 状态按路写入 `runtime/adaptive_alert_state/{source_id}.json`，采用临时文件原子替换，避免 raw/masked 分支并发写坏 JSON。

注意：DeepStream `nvinfer` 不适合在运行中通过反复 `READY -> PLAYING` 重建上下文来切换频率，容易触发 CUDA host buffer 分配错误。本项目已改为运行中直接更新 `interval` 属性。容器验证中 paused 模式已能实际下发 `effective_nvinfer_interval=250`，metadata 会同时输出 `should_process=False`、`requested_fps=0.00` 和 `nvinfer_interval=250`，用于静止画面近似暂停 DeepStream 推理。
...
5. 自适应推理已完成运行时 5fps/25fps 的 `nvinfer interval` 动态切换；静止 paused 模式已验证可实际下发 `effective_nvinfer_interval=250`，用于近似暂停静止画面推理。
